<form class="container" action="{{ $action }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row mb-3">
        ...
        <div class="col">
            <label for="consultant_id" class="form-label">{{ __('Select Consultant') }}</label>
            <select name="consultant_id" id="consultant_id" class="form-control ">
                <option value="">{{ __('Select Consultant') }}</option>
                @foreach ($consultants as $consultant)
                    <option class="form-control" value="{{ $consultant->id }}"
                        {{ old('consultant_id', $reservation->consultant_id) == $consultant->id ? 'selected' : '' }}>
                        {{ __($consultant->name) }}
                    </option>
                @endforeach
            </select>
